...
- **Known accepted gap**: `@xenova/transformers` (local embedding model for policy RAG) pulls in `onnxruntime-web` → `protobufjs`, which has an unpatched critical CVE chain. Fixing it requires `npm audit fix --force`, which downgrades `@xenova/transformers` to `1.4.2` (breaking) — not yet done, needs a dedicated pass to verify the embedding pipeline still works on that version before shipping.
...
- **Live services** (project `internship26-gd`, region `asia-south1`, chosen to sit close to the Supabase DB's `ap-south-1`): backend `https://spms-backend-397963581210.asia-south1.run.app` (deployed via `gcloud run deploy --source`, builds from `backend/Dockerfile` through Cloud Build), frontend `https://spms-frontend-397963581210.asia-south1.run.app` (built locally with `docker build --build-arg NEXT_PUBLIC_API_URL=<backend URL>`, pushed to Artifact Registry at `asia-south1-docker.pkg.dev/internship26-gd/cloud-run-source-deploy/spms-frontend`, deployed via `gcloud run deploy --image` — required this two-step path since `gcloud run deploy --source` has no `--build-arg` flag). Backend's `FRONTEND_URL` env var was updated post-deploy (`gcloud run services update --update-env-vars`) once the frontend's real URL was known — the two services have a circular URL dependency, so backend necessarily deploys first with a placeholder, then gets patched.
- **Still open**: `npm audit fix --force` for the critical `protobufjs` chain (`@xenova/transformers` → `onnxruntime-web`) — breaking, downgrades `@xenova/transformers` to `1.4.2`, needs the embedding pipeline re-verified before applying. Not yet done.
